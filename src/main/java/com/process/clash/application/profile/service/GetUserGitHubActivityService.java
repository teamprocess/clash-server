...
public class GetUserGitHubActivityService implements GetUserGitHubActivityUsecase {
    ...
    @Override
    public GetMyGitHubActivityData.Result execute(GetUserGitHubActivityData.Command command) {
        ...
        List<GetMyGitHubActivityData.Contribution> contributions = rows.stream()
            .map(row -> {
                ...
                return GetMyGitHubActivityData.Contribution.of(row.date().toString(), row.count(), level);
            })
            .toList();
