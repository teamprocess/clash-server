...
class RankingTierUpdateServiceTest {
    ...
    @Test
    @DisplayName("1위이고 exp >= 150000이면 AURA를 부여한다")
    void computeRankTier_rank1_expOver150k_returnsAura() {
        User user = createUser(1L, 150_000, RankTier.NONE);
    ...
    @Test
    @DisplayName("2위이고 exp >= 100000이면 MASTER를 부여한다")
    void computeRankTier_rank2_expOver100k_returnsMaster() {
        User rank1 = createUser(1L, 150_000, RankTier.NONE);
        User rank2 = createUser(2L, 100_000, RankTier.NONE);
    ...
    @Test
    @DisplayName("3위이고 exp >= 100000이면 MASTER를 부여한다")
    void computeRankTier_rank3_expOver100k_returnsMaster() {
        User rank1 = createUser(1L, 150_000, RankTier.NONE);
        User rank2 = createUser(2L, 120_000, RankTier.NONE);
        User rank3 = createUser(3L, 100_000, RankTier.NONE);
    ...
    void computeRankTier_rank4_returnsNone() {
        User rank1 = createUser(1L, 150_000, RankTier.AURA);
        User rank2 = createUser(2L, 120_000, RankTier.MASTER);
        User rank3 = createUser(3L, 100_000, RankTier.MASTER);
        User rank4 = createUser(4L, 100_000, RankTier.NONE);
    ...
    @Test
    @DisplayName("1위이지만 exp < 100000이면 NONE을 부여한다")
    void computeRankTier_rank1_expUnder100k_returnsNone() {
        User user = createUser(1L, 99_999, RankTier.AURA);
        when(userRepositoryPort.findAllOrderByTotalExpDesc()).thenReturn(List.of(user));
        when(seasonRepositoryPort.findCurrentSeason()).thenReturn(Optional.empty());
    ...
    @Test
    @DisplayName("1위이고 exp가 100000~149999이면 MASTER를 부여한다")
    void computeRankTier_rank1_expBetween100kAnd150k_returnsMaster() {
        User user = createUser(1L, 149_999, RankTier.NONE);
        when(userRepositoryPort.findAllOrderByTotalExpDesc()).thenReturn(List.of(user));
        when(seasonRepositoryPort.findCurrentSeason()).thenReturn(Optional.empty());
    ...
    @Test
    @DisplayName("2위이지만 exp < 100000이면 NONE을 부여한다")
    void computeRankTier_rank2_expUnder100k_returnsNone() {
        User rank1 = createUser(1L, 150_000, RankTier.NONE);
        User rank2 = createUser(2L, 99_999, RankTier.NONE);
        when(userRepositoryPort.findAllOrderByTotalExpDesc()).thenReturn(List.of(rank1, rank2));
        when(seasonRepositoryPort.findCurrentSeason()).thenReturn(Optional.empty());
    ...
    void updateRankTiers_unchangedTier_doesNotSaveUser() {
        User user = createUser(1L, 150_000, RankTier.AURA);
        when(userRepositoryPort.findAllOrderByTotalExpDesc()).thenReturn(List.of(user));
        when(seasonRepositoryPort.findCurrentSeason()).thenReturn(Optional.empty());
