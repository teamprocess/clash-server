...
class DeleteSubjectV2ServiceTest {
    ...
    private DeleteSubjectV2Service deleteSubjectV2Service;

    @BeforeEach
    void setUp() {
        deleteSubjectV2Service = new DeleteSubjectV2Service(
    ...
    @Test
    @DisplayName("진행중인 세션이 없으면 과목 그룹을 삭제한다")
    void execute_deletesSubjectWhenNoActiveSession() {
        Actor actor = new Actor(1L);
        RecordSubjectV2 subject = new RecordSubjectV2(10L, 1L, "백엔드", 0L, Instant.now(), Instant.now());
        DeleteSubjectV2Data.Command command = new DeleteSubjectV2Data.Command(actor, 10L);

        when(recordSubjectV2RepositoryPort.findById(10L)).thenReturn(Optional.of(subject));
        when(recordSessionV2RepositoryPort.existsActiveSessionBySubjectId(10L)).thenReturn(false);

        deleteSubjectV2Service.execute(command);

        verify(recordSubjectV2RepositoryPort).deleteById(10L);
    }
    ...
    void execute_throwsWhenActiveSessionExists() {
        ...
        when(recordSubjectV2RepositoryPort.findById(10L)).thenReturn(Optional.of(subject));
        when(recordSessionV2RepositoryPort.existsActiveSessionBySubjectId(10L)).thenReturn(true);

        assertThatThrownBy(() -> deleteSubjectV2Service.execute(command))
            .isInstanceOf(SubjectV2HasActiveSessionException.class);
        verify(recordSubjectV2RepositoryPort, never()).deleteById(10L);
    }
    ...
    void execute_throwsDomainConflictWhenDeleteFailsByIntegrity() {
        ...
        org.mockito.Mockito.doThrow(new DataIntegrityViolationException("fk constraint"))
            .when(recordSubjectV2RepositoryPort).deleteById(10L);
